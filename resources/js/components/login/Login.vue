    ...
    required class="input">
     ...
     <button @click.prevent="login" :class="btn">
       <transition name="done" mode="out-in">
         <span class="label" :key="second_done">
            {{ second_done ? 'Done' : 'Login'}}
         </span>
       </transition>
...
export default {
  ...
  methods:{
     login(){
        ...
        .catch(error => {console.log(error.response.data)
               setTimeout(this.changeFalse,2000);
        })
   },
   changeSignup(){
  ...
  computed:{
      btn(){
          return this.first_done ? "btn_first" : "btn";
      }
  }
}
</script>
...
.done-enter-active, .done-leave-active{
   transition:all .3s ease-in-out;
}
.done-leave-to, .done-enter{
...
.loginform{
    ...
    //このボタンで変化させるタイミングは二回、一回目はloginボタンを押したときspinnerに変化させる、二回目はデータ送信が完了して成功したときに文字をdoneにする
    //一回目で変える内容の詳細はまず.btnをcursor:default .btn.greenをborder-color: rgba( 0, 0, 0, 0.07 );background-color: #999; .btn.contractをborder-radius:50% width:5.2rem .btn.contract .label をopacity:0 .btn.contract .spinner をopacity:1;
    .btn{
    ...
    .btn .spinner,
    .btn .label{
        transition: .3s cubic-bezier(0.175,0.895,0.320,1.275);
    }
    ...
    //.btn_firstとかすると全部書き直さないといけないけど.btn[]とすると必要な分は上書きになる？
